...
# Create a data set containing each patient and the 'date' of his/her
# earliest admission with a fall.
pat_earliest_fall <- core_fall %>%
  group_by(VisitLink) %>%
  summarise(earliest_fall = min(DaysToEvent))

pat_earliest_fall
nrow(pat_earliest_fall)
...
# these records.
pat_level_earliest_fall <- all_records %>%
  filter(DaysToEvent == earliest_fall)

nrow(pat_level_earliest_fall)
is_patient_level(pat_level_earliest_fall)
